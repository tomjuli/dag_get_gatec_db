...
local_config = Variable.get("dag_get_sps_db", deserialize_json=True)
...
def extract_by_query(file_location, report_type, report_name, extract_date, engine, temp_folder, bucket_name, ACCESS_KEY, SECRET_KEY, database_name, output_bucket):
    ...
    upload_file_to_s3(local_file_path=f"{temp_folder}/{report_type}_{report_name}_{extract_date}.parquet", 
                    ...
                    object_key=f"SPSDB/{report_type}/{report_name}/extract_date={extract_date}/{report_type}_{report_name}_{extract_date}.parquet", 
    ...
    if check_table_exists(database_name=database_name, 
        ...
        create_table_if_not_exist(table_name=f"{report_type}_{report_name}", 
                                s3_path=f"{bucket_name}/SPSDB/{report_type}/{report_name}/", 
...
def set_extract_date_for_time_partition(local_config):
    ...
    local_config['extract_date'] = str(datetime.today().replace(day=1).strftime("%Y-%m-%d"))
    
    local_config = json.dumps(local_config, indent=4)

    Variable.set("dag_get_sps_db", local_config)
...
with DAG(
    ...
    on_failure_callback=SmtpNotifier(
        from_email="no-reply-apache-airflow@mnmsugar.com",
        to=default_args_config["email"]
    )
) as dag:
    ...
    with TaskGroup(group_id=f'process_extract_by_query') as tg:
        ...
        for c in local_config['conn_list']:

            task_start_extract_by_query = EmptyOperator(
                task_id=f"start_extract_by_query_{c['db_name']}",
                trigger_rule=TriggerRule.ALL_SUCCESS
            )

            engine = create_engine(c['sql_conn'])

            for f in c['query_file_list']:
